...
// Vercel deployment trigger - pooler connection

app.use(cors());
app.use(express.json({ limit: '50mb' }));
app.use(express.urlencoded({ limit: '50mb', extended: true }));

// If the database is not configured, return a clear error on most API endpoints
app.use((req, res, next) => {
    if (!pool) {
        return res.status(500).json({
            status: 'error',
            message: 'Database configuration missing. Set DATABASE_URL in Vercel environment variables.',
            help: 'If you are deploying to Vercel, add DATABASE_URL under Project Settings → Environment Variables.'
        });
    }
    next();
});
...
app.post('/api/auth/login', async (req, res) => {
    try {
        ...
        try {
            ...
            if (err.code === '42P01') {
                // Table doesn't exist, create it and default admin
                await pool.query(`
        ...
        const userResult = await pool.query(
            'SELECT id, name, email, role, status, avatar, username FROM users WHERE username = $1 AND password = $2 AND status = $3',
...
app.get('/api/users', async (req, res) => {
    try {
        ...
        const result = await pool.query('SELECT id, name, email, role, status, avatar, username, created_at, updated_at FROM users ORDER BY created_at DESC');
...
app.post('/api/users', async (req, res) => {
    try {
        ...
        const result = await pool.query(
            'INSERT INTO users (name, email, username, password, role, status) VALUES ($1, $2, $3, $4, $5, $6) RETURNING id, name, email, role, status, avatar, username, created_at',
            [name, email || null, username, password, role || 'User', status || 'Active']
        );
...
app.put('/api/users/:id', async (req, res) => {
    try {
        ...
        let query, params;
        if (password) {
            query = 'UPDATE users SET name=$1, email=$2, username=$3, password=$4, role=$5, status=$6, updated_at=NOW() WHERE id=$7 RETURNING id, name, email, role, status, avatar, username, updated_at';
            params = [name, email || null, username, password, role || 'User', status || 'Active', id];
        } else {
            query = 'UPDATE users SET name=$1, email=$2, username=$3, role=$4, status=$5, updated_at=NOW() WHERE id=$6 RETURNING id, name, email, role, status, avatar, username, updated_at';
            params = [name, email || null, username, role || 'User', status || 'Active', id];
        }
    ...
    try {
        ...
        const result = await pool.query(`
            UPDATE master_asset_utama SET
                ...
                longitude=$78, latitude=$79, lanal=$80, photos=$81, identifikasi_aset=$82, updated_at=NOW()
            WHERE id=$83 RETURNING *
        `, [
            str(a.jenis_bmn), str(a.kode_satker), str(a.nama_satker), str(a.kode_barang), str(a.nup),
            ...
            str(a.lanal), str(a.photos), str(a.identifikasi_aset),
            id
        ]);
        if (result.rows.length === 0) return res.status(404).json({ error: 'Asset not found' });
        res.json(result.rows[0]);
